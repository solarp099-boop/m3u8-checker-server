...
async function checkStreams() {
  ...
  try {
    ...
    // --- PASO 1: Verificar el estado actual de todos los canales ---
    await Promise.all(streams.map(async (stream) => {
      ...
      if (stream.status !== status) {
        ...
        stream.status = status; // Actualizamos localmente para el Paso 2
      }
    }));

    // --- PASO 2: Sistema de Autorrecuperación (Self-Healing) ---
    const updatedStreams = await collection.find().toArray();
    for (const stream of updatedStreams) {
      // Ignoramos canales que ya son de la Librería de Respaldo y buscamos los activos que estén "offline"
      if (stream.category !== "Librería de Respaldo" && stream.status === "offline") {
        
        // Escapamos caracteres especiales del nombre para evitar errores en la expresión regular
        const escapedName = stream.name.trim().replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        
        // Buscamos un backup del mismo nombre que esté ONLINE, ordenando del más antiguo al más reciente
        ...
        if (backup) {
          // 1. Reemplazamos la URL del canal caído por la del backup funcional y lo pasamos a "online"
          await collection.updateOne(
            { _id: stream._id }, 
            { $set: { url: backup.url, status: "online" } }
          );
          // 2. Eliminamos este backup específico de la librería para consumirlo permanentemente
          await collection.deleteOne({ _id: backup._id });
          
          console.log(`🟢 [Self-Healing] Canal "${stream.name}" recuperado automáticamente usando backup: ${backup.url}`);
        }
      }
    }
      ...
      <script>
        ...
        function updateCatInput(view, selectedCat = "") {
          ...
          if (view === 'main') {
            ...
            bulkCard.classList.remove('hidden-bulk');
            container.innerHTML = \`<input name="category" class="cat-readonly" value="LIBRERÍA DE RESPALDO" readonly> <button class="nav-btn" style="background:var(--success)">Agregar</button>\`;
          } else {
            if (!selectedCat) {
