...
app.get("/admin", async (req, res) => {
  ...
  try {
    ...
    <body>
      ...
      <div style="background: var(--card); padding: 15px; border-radius: 12px; margin-bottom: 20px; display: flex; gap: 10px; align-items: center; border: 1px solid #333;">
        <span style="font-size: 20px;">🔍</span>
        <input type="text" id="globalSearchInput" placeholder="Buscar canal en tiempo real..." 
               style="flex: 1; background: #000; border: 1px solid #444; color: #fff; padding: 12px; border-radius: 8px; font-size: 15px;"
               oninput="ejecutarBusquedaGlobal()">
        <button class="nav-btn" style="background: #222; border: 1px solid #444; display: none;" id="btnClearSearch" onclick="limpiarBusquedaGlobal()">❌ Limpiar</button>
      </div>

      <div id="view-search" class="view-container" style="border-left: 4px solid var(--warn); margin-bottom: 20px; display:none;">
        <h3 style="margin-top: 0; color: var(--warn);">🎯 Resultados de la Búsqueda Global</h3>
      ...
      <script>
        ...
        function showView(view, btn) {
          limpiarBusquedaGlobal(); // Limpia el buscador al cambiar de pestaña
        ...
        function ejecutarBusquedaGlobal() {
          ...
          // Filtra omitiendo estrictamente las Librerías
